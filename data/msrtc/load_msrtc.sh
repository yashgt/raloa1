...
target=msrtc1
MYOPTS="--host=localhost --user=root --password=goatransport --verbose"
...
mysql ${MYOPTS} -D ${target} -e "load data local infile '${etmfolder}/listofroutes.csv' into table listofroutes ${LOADOPTS}(ROUTE_NO,ROUTE_NAME,FROM_STOP_CD,TILL_STOP_CD); show warnings" 
mysql ${MYOPTS} -D ${target} -e "load data local infile '${etmfolder}/listofstopsonroutes.csv' into table listofstopsonroutes ${LOADOPTS}(ROUTE_NO,BUS_STOP_CD,STOP_SEQ); show warnings" 
mysql ${MYOPTS} -D ${target} -e "SET FOREIGN_KEY_CHECKS = 0; load data local infile '${etmfolder}/trips_boarding_alighting.csv' into table listoftrips ${LOADOPTS}(TRIP_NO,ROUTE_NO,BUS_STOP_CD,@arrival_tm,DEPARTURE_TM,@is_boarding_stop, @is_alighting,DEPOT_CD,BUS_TYPE_NM, START_DATE, END_DATE, DAY_OFFSET, STOP_SEQ) set arrival_tm=STR_TO_DATE(@ARRIVAL_TM,'%Y-%m-%d %H:%i:%s') ,is_boarding_stop=(@is_boarding_stop='Y'), is_alighting=(@is_alighting='Y'); show warnings; SET FOREIGN_KEY_CHECKS = 1;"
