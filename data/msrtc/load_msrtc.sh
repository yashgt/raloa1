...
mysql ${MYOPTS} -D ${target} -e "load data local infile '${etmfolder}/listof_routes.csv' into table listofroutes fields terminated by ',' enclosed by '\"' ignore 1 lines(ROUTE_NO,ROUTE_NAME,FROM_STOP_CD,TILL_STOP_CD); show warnings" 
mysql ${MYOPTS} -D ${target} -e "load data local infile '${etmfolder}/listofStopOfRoutes.csv' into table listofstopsonroutes fields terminated by ',' ignore 1 lines(ROUTE_NO,BUS_STOP_CD,STOP_SEQ); show warnings" 
mysql ${MYOPTS} -D ${target} -e "load data local infile '${etmfolder}/listofBoardingAlighting.csv' replace into table listoftrips fields terminated by ',' ignore 1 lines(TRIP_NO,ROUTE_NO,BUS_STOP_CD,@arrival_tm,DEPARTURE_TM,@is_boarding_stop, @is_alighting,DEPOT_CD,BUS_TYPE_NM) set arrival_tm=STR_TO_DATE(@ARRIVAL_TM,'%d-%m-%Y %H:%i:%s') ,is_boarding_stop=(@is_boarding_stop='Y'), is_alighting=(@is_alighting='Y'); show warnings" 
