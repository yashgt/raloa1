...
mysql %MYOPTS% -D msrtc1 -e "load data local infile 'data\\data\\ListOfStops.csv' into table listofstops fields terminated by '\t' ignore 1 lines(BUS_STOP_CD, BUS_STOP_NM); show warnings" 
mysql %MYOPTS% -D msrtc1 -e "load data local infile 'data\\data\\ListOfRoutes.csv' into table listofroutes fields terminated by '\t' ignore 1 lines(ROUTE_NO,ROUTE_NAME,FROM_STOP_CD,TILL_STOP_CD); show warnings" 
