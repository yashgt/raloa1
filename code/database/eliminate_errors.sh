...
mysql --host=${DBHOST} --user=root --password=goatransport -D raloa2 -e "load data local infile 'error_trips.txt' replace into table error_trips(trip_no,depot_cd,error)"
mysql --host=${DBHOST} --user=root --password=goatransport -D raloa2 -e "update error_trips E join msrtc1.listoftrips T on (E.trip_no=T.trip_no) set E.depot_cd=T.depot_cd"
